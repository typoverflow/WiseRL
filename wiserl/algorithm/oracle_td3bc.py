...
class OracleTD3BC(Algorithm):
    ...
    def actor_loss(self, encoded_obs, action, reduce=True):
        ...
        # print(bc_loss.requires_grad)
        # bc_loss = torch.sum((new_actions - action)**2, dim=-1, keepdim=True)
        # print(bc_loss.requires_grad)
        # from wiserl.module.actor import DeterministicActor, GaussianActor
        # if isinstance(self.network.actor, DeterministicActor):
        #     bc_loss = torch.sum((self.network.actor.sample(encoded_obs)[0] - action)**2, dim=-1, keepdim=True)
        # elif isinstance(self.network.actor, GaussianActor):
        #     bc_loss = - self.network.actor.evaluate(encoded_obs, action)[0]
