...
class SALEEmbedding(Algorithm):
    ...
    def train_step(self, batches, step: int, total_steps: int):
        ...
        nexts_embed = AvgL1Norm(self.network.state_encoder(batch['next_obs']))
        print(batch['obs'].shape, batch['next_obs'].shape)
        print(state_embed.shape, nexts_embed.shape)
        joint_embed = self.network.joint_encoder(torch.cat([state_embed[0], batch['action']], dim=-1))
        loss = torch.mean((joint_embed-nexts_embed.detach())**2)
