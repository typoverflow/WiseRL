...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for i in range(M):
            ...
            origin_action = default_traj["action"][index + i][start]
            origin_obs = default_traj["obs"][index + i][start]
            ...
            z_logprob_list.append(z_logprob)
            sum_logprob_list.append(sum_logprob)
            
            # print(z_logprob)
            # print(sum_logprob)
        
        # merge z_logprob and sum_logprob
        z_logprob = torch.cat(z_logprob_list, dim=0)
        sum_logprob = torch.cat(sum_logprob_list, dim=0)
        # print(z_logprob.shape)
        # print(sum_logprob.shape)
